...
class FLIP(torch.nn.Module):
    ...
    def compute_flip(self, reference, test, pixels_per_degree):        
        ...
        # Feature metric
        deltaE_f = torch.max(torch.abs(torch.norm(edges_reference, dim=1) - torch.norm(edges_test, dim=1)), torch.abs(torch.norm(points_test, dim=1) - torch.norm(points_reference, dim=1)))
...
def hyab(reference, test):
    ...
    return abs(delta[:, 0:1, :, :]) + torch.norm(delta[:, 1:3, :, :], dim=1)
